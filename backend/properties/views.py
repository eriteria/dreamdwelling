...
from .models import (
    Property, PropertyType, Feature, PropertyImage, 
    PropertyDocument, PropertyReview, OpenHouse
)
from .serializers import (
    PropertyListSerializer, PropertyDetailSerializer, PropertyTypeSerializer,
    FeatureSerializer, PropertyImageSerializer, PropertyDocumentSerializer,
    PropertyReviewSerializer, OpenHouseSerializer, PropertyCreateUpdateSerializer
)
...
class PropertyViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'description', 'address_line1', 'city', 'state', 'zip_code']
    ...
    def add_images(self, request, pk=None):
        ...
        if is_primary:
            ...
            PropertyImage.objects.filter(property=property_instance, is_primary=True).update(is_primary=False)
    ...
    def add_review(self, request, pk=None):
        ...
        existing_review = PropertyReview.objects.filter(property=property_instance, user=request.user).first()
        if existing_review:
            return Response({'detail': 'You have already reviewed this property.'},
    ...
    def add_open_house(self, request, pk=None):
        ...
        # Check if the user owns this property
        if property_instance.listed_by != request.user:
            return Response({'detail': 'You do not have permission to add open house events to this property.'},
                          status=status.HTTP_403_FORBIDDEN)
